"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SpellService = void 0;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const nspell_1 = __importDefault(require("nspell"));
const normalization_1 = require("./normalization");
class NSpellDictionary {
    constructor(dictionary) {
        this.dictionary = dictionary;
    }
    correct(token) {
        return this.dictionary.correct(token);
    }
    suggest(token) {
        return this.dictionary.suggest(token);
    }
}
class WordSetDictionary {
    constructor(words) {
        this.buckets = new Map();
        this.words = new Set(words);
        for (const word of this.words) {
            const key = WordSetDictionary.getBucketKey(word);
            const list = this.buckets.get(key) ?? [];
            list.push(word);
            this.buckets.set(key, list);
            const shortKey = word.slice(0, 1);
            const shortList = this.buckets.get(shortKey) ?? [];
            shortList.push(word);
            this.buckets.set(shortKey, shortList);
        }
    }
    correct(token) {
        return this.words.has(token);
    }
    suggest(token) {
        const normalized = (0, normalization_1.normalize)(token);
        const primary = this.buckets.get(WordSetDictionary.getBucketKey(normalized)) ?? [];
        const secondary = this.buckets.get(normalized.slice(0, 1)) ?? [];
        const candidates = (primary.length > 0 ? primary : secondary).slice(0, 6000);
        const scored = [];
        for (const candidate of candidates) {
            const lengthDelta = Math.abs(candidate.length - normalized.length);
            if (lengthDelta > 2) {
                continue;
            }
            const distance = boundedLevenshtein(normalized, candidate, 2);
            if (distance > 2) {
                continue;
            }
            scored.push({ word: candidate, distance, lengthDelta });
        }
        scored.sort((a, b) => {
            if (a.distance !== b.distance) {
                return a.distance - b.distance;
            }
            if (a.lengthDelta !== b.lengthDelta) {
                return a.lengthDelta - b.lengthDelta;
            }
            return a.word.localeCompare(b.word);
        });
        return scored.slice(0, 8).map((x) => x.word);
    }
    static getBucketKey(word) {
        if (word.length <= 1) {
            return word;
        }
        return word.slice(0, 2);
    }
}
class SpellService {
    constructor(extensionPath) {
        this.dictionaries = new Map();
        this.huAsciiFoldIndex = new Map();
        this.initialized = false;
        this.extensionPath = extensionPath;
    }
    ensureInitialized() {
        if (this.initialized) {
            return;
        }
        this.loadDictionary('en', 'en_US');
        this.loadDictionary('hu', 'hu_HU');
        this.initialized = true;
    }
    check(token, config) {
        const normalized = (0, normalization_1.normalize)(token);
        const languages = [];
        for (const language of config.languages) {
            const spell = this.dictionaries.get(language.toLowerCase());
            if (!spell) {
                continue;
            }
            if (spell.correct(normalized)) {
                languages.push(language.toLowerCase());
                continue;
            }
            if (language.toLowerCase() === 'hu' &&
                config.treatAsHungarianWhenAsciiOnly &&
                (0, normalization_1.isAsciiOnly)(normalized) &&
                this.huAsciiFoldIndex.has((0, normalization_1.asciiFold)(normalized))) {
                languages.push('hu');
            }
        }
        return { correct: languages.length > 0, languages };
    }
    suggest(token, config) {
        const normalized = (0, normalization_1.normalize)(token);
        const output = [];
        const seen = new Set();
        for (const language of config.languages) {
            const langCode = language.toLowerCase();
            const spell = this.dictionaries.get(langCode);
            if (!spell) {
                continue;
            }
            for (const suggestion of spell.suggest(normalized)) {
                if (seen.has(suggestion.toLowerCase())) {
                    continue;
                }
                seen.add(suggestion.toLowerCase());
                output.push({ replacement: suggestion, confidence: 0.75, sourceDictionary: langCode });
                if (output.length >= config.suggestionsMax) {
                    return output;
                }
            }
            if (langCode === 'hu' &&
                config.treatAsHungarianWhenAsciiOnly &&
                (0, normalization_1.isAsciiOnly)(normalized)) {
                const folded = (0, normalization_1.asciiFold)(normalized);
                const matches = this.huAsciiFoldIndex.get(folded) ?? [];
                for (const suggestion of matches) {
                    if (seen.has(suggestion.toLowerCase())) {
                        continue;
                    }
                    seen.add(suggestion.toLowerCase());
                    output.push({ replacement: suggestion, confidence: 0.9, sourceDictionary: 'hu' });
                    if (output.length >= config.suggestionsMax) {
                        return output;
                    }
                }
            }
        }
        return output;
    }
    loadDictionary(languageCode, dictionaryFolderName) {
        const rootDir = this.resolveDictionaryRoot();
        const baseDir = node_path_1.default.join(rootDir, dictionaryFolderName);
        const affPath = node_path_1.default.join(baseDir, `${dictionaryFolderName}.aff`);
        const dicPath = node_path_1.default.join(baseDir, `${dictionaryFolderName}.dic`);
        if (languageCode.toLowerCase() === 'hu') {
            const words = this.readDictionaryWords(dicPath);
            this.buildHungarianFoldIndex(words);
            try {
                const aff = node_fs_1.default.readFileSync(affPath, 'utf8');
                const dic = node_fs_1.default.readFileSync(dicPath, 'utf8');
                this.dictionaries.set(languageCode, new NSpellDictionary((0, nspell_1.default)(aff, dic)));
            }
            catch (error) {
                this.dictionaries.set(languageCode, new WordSetDictionary(words));
                console.warn(`[KoSpellCheck] Hungarian nspell load failed; using fallback dictionary: ${formatError(error)}`);
            }
            return;
        }
        const aff = node_fs_1.default.readFileSync(affPath, 'utf8');
        const dic = node_fs_1.default.readFileSync(dicPath, 'utf8');
        this.dictionaries.set(languageCode, new NSpellDictionary((0, nspell_1.default)(aff, dic)));
    }
    resolveDictionaryRoot() {
        const candidates = [
            node_path_1.default.join(this.extensionPath, 'resources', 'dictionaries'),
            node_path_1.default.join(this.extensionPath, 'Resources', 'Dictionaries'),
            node_path_1.default.join(this.extensionPath, 'dictionaries')
        ];
        for (const candidate of candidates) {
            if (this.hasDictionaryFiles(candidate)) {
                return candidate;
            }
        }
        throw new Error(`KoSpellCheck dictionaries not found. Checked: ${candidates.join(', ')}`);
    }
    hasDictionaryFiles(root) {
        return (node_fs_1.default.existsSync(node_path_1.default.join(root, 'hu_HU', 'hu_HU.aff')) &&
            node_fs_1.default.existsSync(node_path_1.default.join(root, 'hu_HU', 'hu_HU.dic')) &&
            node_fs_1.default.existsSync(node_path_1.default.join(root, 'en_US', 'en_US.aff')) &&
            node_fs_1.default.existsSync(node_path_1.default.join(root, 'en_US', 'en_US.dic')));
    }
    readDictionaryWords(dicPath) {
        const output = [];
        const lines = node_fs_1.default.readFileSync(dicPath, 'utf8').split(/\r?\n/);
        for (const raw of lines.slice(1)) {
            const trimmed = raw.trim();
            if (!trimmed || trimmed.startsWith('#')) {
                continue;
            }
            const firstField = trimmed.split(/\s+/u, 1)[0];
            const slashIdx = firstField.indexOf('/');
            const word = slashIdx > 0 ? firstField.slice(0, slashIdx) : firstField;
            const normalized = (0, normalization_1.normalize)(word);
            if (!normalized || normalized.includes(' ')) {
                continue;
            }
            output.push(normalized);
        }
        return output;
    }
    buildHungarianFoldIndex(words) {
        this.huAsciiFoldIndex.clear();
        for (const word of words) {
            const folded = (0, normalization_1.asciiFold)(word);
            const list = this.huAsciiFoldIndex.get(folded) ?? [];
            if (!list.includes(word)) {
                list.push(word);
            }
            this.huAsciiFoldIndex.set(folded, list);
        }
    }
}
exports.SpellService = SpellService;
function boundedLevenshtein(a, b, maxDistance) {
    if (a === b) {
        return 0;
    }
    if (Math.abs(a.length - b.length) > maxDistance) {
        return maxDistance + 1;
    }
    let prev = new Array(b.length + 1);
    let curr = new Array(b.length + 1);
    for (let j = 0; j <= b.length; j++) {
        prev[j] = j;
    }
    for (let i = 1; i <= a.length; i++) {
        curr[0] = i;
        let rowMin = curr[0];
        const aCode = a.charCodeAt(i - 1);
        for (let j = 1; j <= b.length; j++) {
            const cost = aCode === b.charCodeAt(j - 1) ? 0 : 1;
            curr[j] = Math.min(prev[j] + 1, curr[j - 1] + 1, prev[j - 1] + cost);
            if (curr[j] < rowMin) {
                rowMin = curr[j];
            }
        }
        if (rowMin > maxDistance) {
            return maxDistance + 1;
        }
        [prev, curr] = [curr, prev];
    }
    return prev[b.length];
}
function formatError(error) {
    let raw;
    if (error instanceof Error) {
        raw = error.message;
    }
    else {
        raw = String(error);
    }
    const firstLine = raw.split(/\r?\n/u, 1)[0].trim();
    if (firstLine.length <= 320) {
        return firstLine;
    }
    return `${firstLine.slice(0, 320)}...`;
}
//# sourceMappingURL=spellService.js.map